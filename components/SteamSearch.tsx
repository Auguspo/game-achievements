...
const SteamSearch: React.FC = () => {
  ...
  const [data, setData] = useState<{ personaname: string; avatarfull: string } | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response.ok && result.response && result.response.players.length > 0) {
        ...
        setData({ personaname: player.personaname, avatarfull: player.avatarfull });
        setShowGames(true);
      } else {
      ...
      setShowGames(false);
      setError(`An error occurred while searching for the Steam data: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex flex-col items-center space-y-4">
      ...
      {data && (
        <div className="mt-4 flex flex-col items-center">
          ...
          <img src={data.avatarfull} alt={`${data.personaname} avatar`} className="w-32 h-32 rounded-full mt-2" />
        </div>
      )}
